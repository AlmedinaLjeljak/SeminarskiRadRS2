...
class _TerminDetailScreenState extends State<TerminDetailScreen> {
  ...
  void initState() {
    ...
    _fetchKlijent();
    _fetchTerminiForKlijent(_modifiedKlijentId ?? -1);
    _fetchOccupiedAppointments();
  }
  ...
  Future<void> _fetchKlijent() async {
    try {
      ...
      setState(() {
        ...
        if (result?.result.isNotEmpty == true) {
          _selectedKlijent = _modifiedKlijentId ?? result!.result[0].korisnikId;
          _modifiedKlijentId = _selectedKlijent;
        }
      });
    } catch (e) {
      print(e);
  ...
  Future<int?> getUposlenikId() async {
    try {
      final korisnici = await _korisniciProvider.get(filter: {
        'korisnickoIme': 'desktop', // Ovdje promijeni ako imaš dinamički username
      });
      ...
      print('Greška pri preuzimanju ID-a korisnika: $e');
      return null;
    }
  }
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(height: 16.0),
                  DropdownButton<int>(
                    value: _modifiedKlijentId,
                    onChanged: (newValue) {
                      setState(() {
                        _modifiedKlijentId = newValue!;
                        _selectedKlijent = newValue;
                        _isSaveButtonEnabled = true;
                      });
                    },
  ...
  void _saveModifiedTermin() async {
    ...
    if (_isDateTimeOccupied(selectedDateTime)) {
      ...
      widget.termin!.datum = _modifiedDatum;
      widget.termin!.korisnikIdKlijent = _modifiedKlijentId;
      widget.termin!.korisnikIdUposlenik = await getUposlenikId();
  ...
  void _saveNewTermin() async {
    ...
    if (_isDateTimeOccupied(selectedDateTime)) {
      ...
      try {
        final insertedTermin = await _terminiProvider.insert(newTermin);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Appointment successfully added.'),
            backgroundColor: Colors.green,
